...
class Car:
    ...
    def get_state(self, track):
        ...
        return state.unsqueeze(0)  # Ensure batch dimension
...
def train():
    ...
    track = Track()
    car = Car()
    load_model(car.brain)  # Load existing model if available
    rewards = []
    running = True
    ...
    save_model(car.brain)  # Save trained model
    pygame.quit()
